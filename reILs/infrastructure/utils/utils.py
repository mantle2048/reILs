...
def concat_listofrollouts(rollouts):
    """
        Take a list of rollout dictionaries and return
        one rollout dictionary, where each dictionary value is
        a concatenation of that array from across the rollouts
    """
    concatenated_rollouts = {}
    for k in rollouts[0].keys():
        concatenated_rollouts[k] = \
            np.concatenate([rol[k] for rol in rollouts])
    return concatenated_rollouts
